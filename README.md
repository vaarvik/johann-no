...
Welcome to the code of [johann.no](https://johann.no). This README provides an overview of the project structure,
...
The project is a web application designed to show Johann. It utilizes Next.js and Vercel for deployment.
...
1. ```bash
   ...
   ```
2. ```bash
   chmod ug+x .husky/* && chmod ug+x .husky/_/*
   ```
3. ```bash
   npm install
   ```
4. Set up environment variables by creating a .env file from `.env.example` and updating it with your values.
...
- `./.eslint.config.mjs`: ESLint configuration for JavaScript and TypeScript linting.
